...
public class RabbitMQConsumer implements Flow.Subscription {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "inventory.handles", durable = "true"),
            exchange = @Exchange(value = "core-ddd-challenge", type = "topic"),
            key = "ddd.inventory.#"
    ))
    ...
    private void localReplay(Message<String> peyload) {
        try {
            ...
            logger.log(Level.INFO, "###### Message received from {0} -- {1}", new String[]{event.type, event.getClass().getName()});
